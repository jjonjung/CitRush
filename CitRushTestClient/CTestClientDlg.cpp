...
void CTestClientDlg::OnBtnScnIntercept() {
    // 팩맨 HP 양호, 플레이어 중거리 접근 → INTERCEPT(3) 예상
...
void CTestClientDlg::OnBtnScnPellet() {
    // P-Pellet 쿨다운 0, 팩맨 일반 상태 → CONSUME_PELLET(7) 예상
...
void CTestClientDlg::OnBtnScnSplit() {
    // 플레이어들 여러 방향, 클론 분산 유리 → SPLIT_FORMATION(10) 예상
...
void CTestClientDlg::Log(const CString& msg) {
    ...
    m_listLog.InsertString(0, entry);   // 최신 항목이 위에
    // 로그 300개 초과 시 오래된 것 제거
